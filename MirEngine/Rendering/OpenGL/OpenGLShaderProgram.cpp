...
bool OpenGLShaderProgram::build(const std::string& vertexSource,
...
void OpenGLShaderProgram::bind() const noexcept
{
    if (program_ != 0)
        glUseProgram(static_cast<GLuint>(program_));
}

int OpenGLShaderProgram::uniformLocation(const char* name) const noexcept
{
...
void OpenGLShaderProgram::setMat4(int location, const double* matrix) const noexcept
{
