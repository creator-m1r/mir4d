...
bool Logger::shouldLog(LogLevel level) const {
    // Если логирование полностью отключено — не показываем ничего.
...
void Logger::output(LogLevel level, const std::string& message) {
    ...
    if (level == LogLevel::Error || level == LogLevel::Critical) {
        ...
        // Ничего не делаем при Off — но это сюда не должно попадать из-за shouldLog
    } else {
...
void Logger::log(LogLevel level, const std::string& message) {
    // Захватываем замок, чтобы только один поток мог писать одно��ременно.
