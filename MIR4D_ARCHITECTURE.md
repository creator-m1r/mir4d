...
Spatial Menu работает исключительно через `MIRIntent → MIRIntentRouter → Command → MirEngine`; он не содержит CAD-алгоритмов, геометрии, Vision, голосового движка и GPU-кода. Активация: клавиша `]`, средняя кнопка мыши, двухпальцевый hold (iPad, каркас). Голос: распознавание в `MIR4DVoiceAssistant`, адаптер публикует `MIRIntent(source: .voice)`. Рука: подписка на `MIRHandGestureModule.shared.session.intentPublisher` → резолвер `MIR4DInteractionContext` (жест + контекст сцены → семантическое действие) → `MIRIntent(source: .spatial, action: <semanticAction>)`. Sculpt несёт типизированный `MIR4DSculptIntent` (position/depth/direction/pressure/radius/strength/velocity/mode), собираемый из `MIRHandIntent` на слое App; `depth` — реальная сценическая глубина (Vision `z` через `MIRHandSpatialMapper`), не оценка. Живая сквозная связка (п.10 ТЗ): при цели `.sculpt` адаптер публикует `MIR4DSculptIntent` в `MIR4DSculptIntentPublisher` → `MIR4DSculptCommandBridge` проецирует объём жеста на выбранное тело (точка кисти — ray-cast камеры вьюпорта `MirEnginePickWorldPoint`, откат на bounding box при промахе луча) и вызывает `MIR4DModelRuntime.deformSelected` → `MirEngineDeformSelected` (реальное смещение вершин меша выбранного объекта, режимы push/pull/smooth/inflate/grab/pinch/cut/paint, пересчёт нормалей). Команда `model.sculpt` включает режим скульпта. Штрих отменяем целиком: `MIR4DSculptCommandBridge` снимает слепок меша через `MirEngineBeginDeformSelected` в начале штриха и фиксирует ровно одну `DeformObjectCommand` в `SceneCommandHistory` через `MirEngineEndDeformSelected` по простою жеста (Ctrl+Z отменяет весь штрих, а не кадр). Активируется из радиального меню «Объект → Тело → Воздушный скульпт» (команда `model.sculpt`) при выделенном теле в КУ Model; `MIRSpatialMenuController` держит `interactionTarget` адаптера в синхроне с `selectedTool`/выделением, поэтому pinch над телом сразу лепит, а не открывает меню. Каждая поза руки — отдельный режим (`MIR4DSculptIntent.Mode(handPose:)`): pinch→push, fist→pull, point→smooth, openPalm→inflate, grab→grab, thumbsUp→pinch, vSign→cut, twoFinger→paint; позы без смысла (rest/threeFinger) не деформируют. При активации из CAD-дерева команда синхронизирует MirEngine-выделение через `MirEngineSelectObject`, так что скульпт работает независимо от способа выбора тела (клик во вьюпорте или дерево). На шине — `MIRIntent(action: "sculpt")`. Создание эскизов руками: в Sketch-КУ (`interactionTarget == .sketch`, выставляется когда `appState.workbench == .sketch`) указательный жест (`point`) накапливает вершины штриха в нормализованном объёме жестов; каждый кадр публикуется `MIR4DSketchIntent` (live-превью), на отпускании — финальный. `MIR4DSketchCommandBridge` проецирует вершины (-1..1) на плоскость эскиза (half=100 мм) и пушит как 2D-оверлей через `MirEnginePushSketch`, передавая базис выбранной плоскости (`MIR4DModelRuntime.activeSketchPlane`, зеркало `appState.sketchPlane`); поддерживаются XY/YZ/ZX, несколько штрихов аккумулируются в один оверлей. Единая шина намерений — `MIRIntentRouter` (§33); легаси-дубликаты `MIR4DHandIntent` / `MIR4DSpatialSceneIntent` / `MIR4DHandIntentBus` удалены.
