...
struct MIR4DLaunchProjectSelectionView: View {
    ...
    private var recentProjects: [MIR4DRecentProject] { MIR4DProjectSession.shared.recentProjectsList() }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showNewProject) { MIR4DNewProjectView().environmentObject(appState) }
        .onAppear {
    ...
    private var brandCard: some View {
        ...
        .background(RoundedRectangle(cornerRadius: 12).fill(
            LinearGradient(colors: [Color(red: 0.035, green: 0.055, blue: 0.085), Color(red: 0.010, green: 0.016, blue: 0.026)], startPoint: .topLeading, endPoint: .bottomTrailing)
        ))
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.white.opacity(0.20), lineWidth: 1))
        .overlay(alignment: .trailing) {
            Rectangle().fill(LinearGradient(colors: [.clear, .blue.opacity(0.72), .clear], startPoint: .top, endPoint: .bottom)).frame(width: 2).blur(radius: 4)
        }
        .shadow(color: .blue.opacity(0.14), radius: 34, x: 12, y: 0)
        .offset(x: appeared ? 0 : -90).opacity(appeared ? 1 : 0)
    }
    ...
    private var diagnostics: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                ...
                Text("САМОИАГНОСТИКА").font(.system(size: 10, weight: .medium, design: .monospaced)).foregroundStyle(.white.opacity(0.60))
                Spacer()
                Text("\(Int(diagnostic.progress * 100))%").font(.system(size: 10, weight: .medium, design: .monospaced)).foregroundStyle(.white.opacity(0.72))
            }
            Text(diagnostic.currentTitle).font(.system(size: 11, weight: .medium)).foregroundStyle(.white.opacity(0.82)).lineLimit(1)
            ProgressView(value: diagnostic.progress, total: 1).tint(.blue).animation(.easeInOut(duration: 0.2), value: diagnostic.progress)
            if diagnostic.warningCount > 0 {
                Text("\(diagnostic.warningCount) предупреждение").font(.system(size: 9, design: .monospaced)).foregroundStyle(.yellow.opacity(0.8))
            }
        }
    }
    ...
    private var projectDoor: some View {
        ...
        .overlay(alignment: .leading) {
            Rectangle().fill(LinearGradient(colors: [.blue.opacity(0.85), .clear], startPoint: .leading, endPoint: .trailing)).frame(width: 2).shadow(color: .blue.opacity(0.9), radius: 9)
        }
    ...
    private var projectHeader: some View {
        VStack(alignment: .leading, spacing: 5) {
            Text("МИР 4D").font(.system(size: 24, weight: .medium, design: .rounded)).tracking(1.0).foregroundStyle(.white)
            Text("Инженерная среда моделирования").font(.system(size: 10)).foregroundStyle(.white.opacity(0.46))
        }
    ...
    private var mainProjectColumn: some View {
        VStack(alignment: .leading, spacing: 18) {
            Text("НАЧАЛО РАБОТЫ").font(.system(size: 10, weight: .semibold, design: .monospaced)).foregroundStyle(.white.opacity(0.50))
            HStack(spacing: 8) {
            ...
            Button { showOpenProject = true } label: {
                ...
                .font(.system(size: 11, weight: .medium)).foregroundStyle(.white.opacity(0.86)).padding(.horizontal, 14).frame(height: 42)
            ...
            recentProjectsView
            Spacer(minLength: 20)
            HStack(spacing: 18) { smallFooter("gearshape", "Настройки"); smallFooter("book", "Документация") }.foregroundStyle(.white.opacity(0.45))
        }
        .padding(.horizontal, 26).padding(.vertical, 26).frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
    }

    private var recentProjectsView: some View {
        VStack(alignment: .leading, spacing: 9) {
            Text("НЕДАВНИЕ ПРОЕКТЫ").font(.system(size: 9, weight: .semibold, design: .monospaced)).foregroundStyle(.white.opacity(0.42))
            ForEach(Array(recentProjects.prefix(3))) { project in
                Button { MIR4DProjectCommands.shared.open(appState: appState, url: project.url) } label: {
                    HStack(spacing: 9) {
    ...
    private var scenariosColumn: some View {
        VStack(alignment: .leading, spacing: 24) {
            Text("СЦЕНАРИИ РАБОТЫ").font(.system(size: 9, weight: .semibold, design: .monospaced)).foregroundStyle(.white.opacity(0.42))
    ...
    private func actionButton(title: String, subtitle: String, icon: String, accent: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .foregroundStyle(.white).padding(11).frame(maxWidth: .infinity, minHeight: 72, alignment: .topLeading)
