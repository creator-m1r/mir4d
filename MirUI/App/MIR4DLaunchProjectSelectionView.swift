...
struct MIR4DLaunchProjectSelectionView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            withAnimation(.timingCurve(0.16, 0.82, 0.22, 1.0, duration: 0.85)) {
                appeared = true
            }
        }
    }
    ...
    private var brandCard: some View {
                ...
                .fill(
                    LinearGradient(
                        colors: [
                            ...
                            Color(red: 0.010, green: 0.016, blue: 0.026)
                        ],
        ...
        .overlay(alignment: .trailing) {
            Rectangle()
                .fill(
                    LinearGradient(
                        colors: [.clear, .blue.opacity(0.72), .clear],
                        startPoint: .top,
                        endPoint: .bottom
                    )
                )
    ...
    private var diagnostics: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                Circle()
                    .fill(diagnostic.state == .failed ? .red : .blue)
    ...
    private var projectDoor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 10)
                .fill(Color(red: 0.018, green: 0.024, blue: 0.036).opacity(0.97))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 10)
        ...
        .overlay(alignment: .leading) {
            Rectangle()
                .fill(
                    LinearGradient(
                        colors: [.blue.opacity(0.85), .clear],
                        startPoint: .leading,
                        endPoint: .trailing
                    )
                )
                .frame(width: 2)
                .shadow(color: .blue.opacity(0.9), radius: 9)
        }
    ...
    private var mainProjectColumn: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            HStack(spacing: 8) {
                ...
                actionButton(title: "Создать", subtitle: "проект", icon: "plus", accent: false) {
                    showNewProject = true
                }
            }
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            ForEach(Array(recentProjects.prefix(3))) { project in
                Button {
                    ...
                    .frame(height: 43)
                    .background(RoundedRectangle(cornerRadius: 6).fill(Color.white.opacity(selectedProjectID == project.id ? 0.07 : 0.035)))
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 7)
                    .fill(accent ? Color.blue.opacity(0.88) : Color.white.opacity(0.045))
            )
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 5)
                .stroke(Color.blue.opacity(glow ? 0.9 : 0.55), lineWidth: 2)
