...
struct MIR4DStartMenuView: View {
    ...
    @State private var showNewProjectPanel = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showOpenPanel) {
            MIR4DProjectOpenView().environmentObject(appState)
        }
        .sheet(isPresented: $showNewProjectPanel) {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                Text("Начало работы")
                    ...
                    .foregroundStyle(.white)
                Text("Выберите направление работы MIR 4D")
                    .font(.system(size: 13))
                    .foregroundStyle(.secondary)
