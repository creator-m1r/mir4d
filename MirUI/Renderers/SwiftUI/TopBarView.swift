...
struct TopBarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            CADRibbonToolGroupsView(appState: appState, registry: registry)
    ...
    private var applicationHeader: some View {
        HStack(spacing: 0) {
            ...
            HStack(spacing: 7) {
                ...
                Text(russian ? "Рабочая среда" : "Workbench").font(MirTheme.Typography.status).foregroundStyle(MirTheme.Colors.textTertiary)
                Text(russian ? appState.workbench.titleRU : appState.workbench.titleEN).font(MirTheme.Typography.bodySemibold).foregroundStyle(MirTheme.Colors.textPrimary)
            }
            .padding(.horizontal, 10).padding(.vertical, 5).background(MirTheme.Colors.surfaceRaised, in: Capsule())
            Button { interfaceCustomizationPresented = true } label: {
                Image(systemName: "rectangle.3.group").font(.system(size: 12, weight: .semibold)).frame(width: 30, height: 28)
            }
        ...
        .padding(.horizontal, MirTheme.Spacing.lg).frame(height: 42).background(MirTheme.Colors.topBar)
    ...
    private func applicationMenu(_ title: String, icon: String, @ViewBuilder content: () -> some View) -> some View {
        Menu { content() } label: {
            Label(title, systemImage: icon).font(MirTheme.Typography.caption).foregroundStyle(MirTheme.Colors.textSecondary).padding(.horizontal, 9).frame(height: 30)
    ...
    private var mainToolbar: some View {
        ...
        .padding(.horizontal, MirTheme.Spacing.lg).frame(height: 46).background(MirTheme.Colors.surface)
    }
    ...
    private var historyButtons: some View {
        HStack(spacing: 2) { topButton("arrow.uturn.backward", russian ? "Отменить" : "Undo") { execute("history.undo") }; topButton("arrow.uturn.forward", russian ? "Повторить" : "Redo") { execute("history.redo") } }
    }
    ...
    private func topButton(_ icon: String, _ label: String, action: @escaping () -> Void) -> some View {
        Button(action: action) { Image(systemName: icon).font(.system(size: 12, weight: .medium)).frame(width: 30, height: 28) }
            .buttonStyle(.plain).foregroundStyle(MirTheme.Colors.textSecondary).background(MirTheme.Colors.surfaceRaised, in: RoundedRectangle(cornerRadius: MirTheme.Radius.small)).help(label)
    }
