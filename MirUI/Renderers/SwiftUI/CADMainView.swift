...
struct CADMainView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .onReceive(NotificationCenter.default.publisher(for: .mir4DRadialMenuBegan)) { note in
                ...
                radialCenter = CGPoint(x: x * proxy.size.width, y: y * proxy.size.height)
                radialVector = .zero; radialMenuPresented = true
            }
            ...
            .onReceive(NotificationCenter.default.publisher(for: .mir4DRadialMenuEnded)) { _ in radialMenuPresented = false; radialVector = .zero }
        }
        ...
        .onAppear { registry.registerDefaults(appState: appState); registry.registerExtendedScenarioCommands(appState: appState); FloatingPanelManager.shared.sync(appState: appState) }
    ...
    private var mainLayout: some View {
        ZStack(alignment: .bottom) {
            centerColumn
            MinimalWorkspaceBar(appState: appState, onCommandPalette: { commandPalettePresented = true }, onSettings: { radialSettingsPresented = true }, onPanels: { workspace.isMinimalMode.toggle() })
    ...
    private var centerColumn: some View {
        ZStack {
            ...
            navigationOverlay
            emptyStateHint
            if appState.workbench == .sketch { sketchLayer; if appState.sketchPlane == nil { SketchPlaneChooser(onPick: { appState.sketchPlane = $0 }) } }
        }
    ...
    private var sketchLayer: some View { SketchViewportView().environmentObject(appState).frame(maxWidth: .infinity, maxHeight: .infinity).allowsHitTesting(true) }

    private var viewport: some View {
        GeometryReader { proxy in
            ViewportRepresentable(appState: appState, onSelectionChanged: { objectID in appState.setSelection(ids: objectID > 0 ? ["\(objectID)"] : [], kind: objectID > 0 ? .body : .none) }, onIOError: { message in appState.showNotification(message, type: .error) }, onCameraOrientationChanged: { theta, phi, distance in
    ...
    private var viewportOverlay: some View {
        ZStack {
            FourDSceneOverlayView(appState: appState).frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading).padding(.top, 44).allowsHitTesting(false)
            if appState.workbench == .fourD { DigitalWorldHUD(appState: appState, store: productionStore); ProductionWorldView(appState: appState, store: productionStore).frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .bottom) }
        }
    }

    private var navigationOverlay: some View {
        NavigationSphereView(theta: cameraTheta, phi: cameraPhi, distance: cameraDistance, isOrthographic: isOrthographic).opacity(0.65).scaleEffect(0.56).fixedSize().frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topTrailing).padding(.top, 14).padding(.trailing, 14)
    }
    ...
    @ViewBuilder private var emptyStateHint: some View {
        if showEmptyState, appState.treeData.first?.children.isEmpty ?? true {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 10) { Button { createDefaultBox() } label: { Label(appState.ui.language == .russian ? "Новое тело" : "New Body", systemImage: "cube.transparent") }.buttonStyle(.borderedProminent).controlSize(.small).tint(MirTheme.Colors.accent); Button { showImporter = true } label: { Label(appState.ui.language == .russian ? "Импорт" : "Import", systemImage: "square.and.arrow.down") }.buttonStyle(.bordered).controlSize(.small) }
            }
            .padding(28).background(.ultraThinMaterial, in: RoundedRectangle(cornerRadius: MirTheme.Radius.floating)).overlay { RoundedRectangle(cornerRadius: MirTheme.Radius.floating).stroke(MirTheme.Colors.panelBorder.opacity(0.8), lineWidth: 1) }.shadow(color: .black.opacity(0.25), radius: 18, y: 8).frame(maxWidth: .infinity, maxHeight: .infinity)
        }
    }

    private var radialMenuOverlay: some View {
        RadialMenuView(store: RadialMenuSettingsStore.shared, center: radialCenter, vector: radialVector, onToolActivated: { tool in
            NotificationCenter.default.post(name: .mir4DRadialMenuEnded, object: nil, userInfo: ["command": tool.command])
...
private struct MinimalWorkspaceBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Button(action: onPanels) { Image(systemName: "sidebar.left") }.help("Дополнительные панели")
            Divider().frame(height: 20)
            Text(appState.documentTitle).font(.system(size: 12, weight: .semibold)).lineLimit(1)
            Spacer(minLength: 16)
            Button(action: onCommandPalette) { Image(systemName: "magnifyingglass") }.help("Поиск команды")
            Button(action: onSettings) { Image(systemName: "gearshape") }.help("Настройки")
        }
        .foregroundStyle(.white.opacity(0.86))
        .padding(.horizontal, 18).padding(.vertical, 11).frame(minHeight: 48)
