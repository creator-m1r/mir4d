...
struct MIR4DCreativeWorkspaceView: View {
    ...
    var body: some View {
        ...
        .fileImporter(
            ...
            allowedContentTypes: mir4DCreativeImportTypes,
        ...
        .onReceive(NotificationCenter.default.publisher(for: .mir4DRadialMenuBegan)) { note in
            guard let x = note.userInfo?["x"] as? Double,
        ...
        ZStack {
            ...
            if appState.workbench == .sketch {
                ...
                if appState.sketchPlane == nil {
                    SketchPlaneChooser { appState.sketchPlane = $0 }
                }
            }
        }
    }
    ...
    private var emptyState: some View {
        if showEmptyState, appState.treeData.first?.children.isEmpty ?? true {
            VStack(spacing: 10) {
                ...
                Text(russian ? "Создайте что-нибудь" : "Create something")
                    .font(.system(size: 20, weight: .medium))
                    .foregroundStyle(.white.opacity(0.70))
                Text(russian ? "Сцена — ваше рабочее пространство" : "The scene is your workspace")
                    .font(.system(size: 11))
                    .foregroundStyle(.white.opacity(0.38))
                HStack(spacing: 8) {
                    Button { createDefaultBox() } label: {
                        Label(russian ? "Новое тело" : "New Body", systemImage: "cube.transparent")
                    }
                    ...
                    Button { showImporter = true } label: {
                        Label(russian ? "Импорт" : "Import", systemImage: "square.and.arrow.down")
                    }
    ...
    private var bottomDock: some View {
        VStack(spacing: 7) {
            HStack(spacing: 5) {
                dockItem("cube.transparent", russian ? "Модель" : "Model", active: appState.workbench == .model) {
                    appState.selectWorkbench(.model)
                }
                dockItem("pencil.and.ruler", russian ? "Эскиз" : "Sketch", active: appState.workbench == .sketch) {
                    appState.selectWorkbench(.sketch)
                }
                dockItem("square.stack.3d.up", russian ? "Сборка" : "Assembly", active: appState.workbench == .assembly) {
                    appState.selectWorkbench(.assembly)
                }
                dockItem("clock.arrow.circlepath", "4D", active: appState.workbench == .fourD) {
                    appState.selectWorkbench(.fourD)
                }
                dockItem("waveform.path.ecg", russian ? "Расчёт" : "Simulation", active: appState.workbench == .simulation) {
                    appState.selectWorkbench(.simulation)
                }
                dockDivider
                dockItem("sparkles", "AI", active: appState.panelState.contains(.aiInspector)) {
                    appState.togglePanel(.aiInspector)
                }
                dockItem("folder", russian ? "Проект" : "Project", active: false) {
                    NotificationCenter.default.post(name: .mir4DProjectClosed, object: nil)
                }
                dockItem("magnifyingglass", "⌘K", active: false) {
    ...
    private var dockDivider: some View {
        Rectangle().fill(.white.opacity(0.10)).frame(width: 1, height: 26).padding(.horizontal, 4)
    }
