...
//
// Когда на холсте выделено два или более виджетов, появляется
// плавающая панель с кнопками выравнивания:
//   [⬅️] [⬆️] [⬇️] [➡️] [🔄↔️] [🔄↕️]
//   Left, CenterH, Right, Top, CenterV, Bottom, DistributeH, DistributeV
//
// При нажатии кнопки вызывается C-функция MirUI_AlignWidgets
// с массивом ID выделенных виджетов и строкой стратегии.
// После выполнения RenderFrame холст и инспектор обновляются.
//
// Панель появляется автоматически, когда selectedIds.count >= 2,
// и исчезает при снятии выделения.
//
// Чистый SwiftUI, использует C-функции моста.

import SwiftUI

// MARK: - ViewModel панели выравнивания (использует тот же vm, что и холст)
...
struct AlignmentToolbarView: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Button { vm.distributeH() } label: {
                Image(systemName: "arrow.left.and.right.square")
            }
            .help("Распределить по горизонтали")
            
            Button { vm.distributeV() } label: {
...
// MARK: - C-функция (сигнатура)

@_silgen_name("MirUI_AlignWidgets")
func MirUI_AlignWidgets(_ widgetIds: UnsafeRawPointer, _ count: Int32, _ strategy: UnsafePointer<CChar>)
