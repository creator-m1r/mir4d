...
/// Профессиональная навигационная сфера МИР 4D.
///
/// В отличие от старой визуализации эта версия строит экранное положение
/// мировых направлений из фактического направления камеры (theta/phi).
/// Поэтому С/Ю/В/З, верх, низ и маркеры направлений всегда согласованы
/// с viewport и не используют приблизительные 2D-коэффициенты.
struct MIR4DNavigationSphereView: View {
    var theta: Double
    var phi: Double
    var distance: Double
    ...
    private let directions: [(MirCameraPreset, SIMD3<Double>)] = [
        (.front, SIMD3(0, 0, 1)), (.frontRight, SIMD3(1, 0, 1).normalized),
        (.right, SIMD3(1, 0, 0)), (.backRight, SIMD3(1, 0, -1).normalized),
        (.back, SIMD3(0, 0, -1)), (.backLeft, SIMD3(-1, 0, -1).normalized),
        (.left, SIMD3(-1, 0, 0)), (.frontLeft, SIMD3(-1, 0, 1).normalized)
    ]
    ...
    private func sphereGrid(radius: CGFloat) -> some View {
        ZStack {
            ...
            Ellipse().stroke(.white.opacity(0.07), lineWidth: 0.7).frame(width: radius * 2, height: radius * 0.42).offset(y: radius * 0.38)
            Rectangle().fill(MirTheme.Colors.accentBright.opacity(0.22)).frame(width: 1, height: radius * 1.8)
        }
        .allowsHitTesting(false)
    }
    ...
    private func worldDirectionMarkers(center: CGPoint, radius: CGFloat) -> some View {
        ZStack {
            ForEach(Array(directions.enumerated()), id: \.offset) { _, item in
                ...
                let visible = max(0.22, min(1.0, (projected.depth + 1) * 0.5))
    ...
    private func project(_ world: SIMD3<Double>, radius: CGFloat) -> ProjectedDirection {
        ...
        return ProjectedDirection(
            x: CGFloat(dot(world, right)) * radius,
            y: CGFloat(dot(world, up)) * radius,
            depth: dot(world, forward)
        )
    }
    ...
    private func snapToNearestViewIfClose() {
        ...
        for preset in MirCameraPreset.allCases {
            let d = preset.direction.normalized
            let score = dot(current, d)
