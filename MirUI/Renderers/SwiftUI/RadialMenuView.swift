...
struct RadialMenuTool: Codable, Identifiable, Hashable {
    var id: UUID = UUID(); var title: String; var icon: String; var command: String
}

struct RadialMenuPanel: Codable, Identifiable, Hashable {
    var id: UUID = UUID(); var title: String; var icon: String; var enabled: Bool = true; var tools: [RadialMenuTool]
}
...
struct RadialMenuSettings: Codable {
    ...
    static let defaultPanels: [RadialMenuPanel] = [
        RadialMenuPanel(title:"Создать",icon:"plus",tools:[
            RadialMenuTool(title:"Тело",icon:"cube.transparent",command:"create.body"),
            RadialMenuTool(title:"Эскиз",icon:"pencil.and.ruler",command:"create.sketch"),
            RadialMenuTool(title:"Форма",icon:"shape",command:"create.form"),
            RadialMenuTool(title:"Компонент",icon:"shippingbox",command:"assembly.component")]),
        RadialMenuPanel(title:"Изменить",icon:"slider.horizontal.3",tools:[
            RadialMenuTool(title:"Размер",icon:"ruler",command:"modify.dimension"),
            RadialMenuTool(title:"Форма",icon:"wand.and.stars",command:"modify.form"),
            RadialMenuTool(title:"Положение",icon:"arrow.up.and.down.and.arrow.left.and.right",command:"transform.move"),
            RadialMenuTool(title:"Связь",icon:"link",command:"modify.constraint")]),
        RadialMenuPanel(title:"Соединить",icon:"link",tools:[
            RadialMenuTool(title:"Связать",icon:"link",command:"assembly.constraint"),
            RadialMenuTool(title:"Собрать",icon:"square.stack.3d.up",command:"assembly.component"),
            RadialMenuTool(title:"Разделить",icon:"rectangle.split.3x1",command:"modify.split")]),
        RadialMenuPanel(title:"Измерить",icon:"ruler",tools:[
            RadialMenuTool(title:"Расстояние",icon:"arrow.left.and.right",command:"measure.distance"),
            RadialMenuTool(title:"Угол",icon:"angle",command:"measure.angle"),
            RadialMenuTool(title:"Размер",icon:"ruler",command:"measure.dimension")]),
        RadialMenuPanel(title:"Посмотреть",icon:"eye",tools:[
            RadialMenuTool(title:"Показать всё",icon:"arrow.up.left.and.arrow.down.right",command:"view.fit"),
            RadialMenuTool(title:"Изометрия",icon:"cube",command:"view.isometric"),
            RadialMenuTool(title:"Проекция",icon:"square.split.2x2",command:"view.orthographic")]),
        RadialMenuPanel(title:"Время",icon:"clock.arrow.circlepath",tools:[
            ...
            RadialMenuTool(title:"Изменение",icon:"waveform.path.ecg",command:"fourD.change")]),
        RadialMenuPanel(title:"Проект",icon:"folder",tools:[
            RadialMenuTool(title:"Открыть",icon:"folder",command:"project.open"),
            RadialMenuTool(title:"Сохранить",icon:"square.and.arrow.down",command:"file.save"),
            RadialMenuTool(title:"Экспорт",icon:"square.and.arrow.up",command:"file.export")]),
        RadialMenuPanel(title:"Назад",icon:"arrow.uturn.backward",tools:[
            RadialMenuTool(title:"Отменить",icon:"arrow.uturn.backward",command:"edit.undo"),
            RadialMenuTool(title:"Повторить",icon:"arrow.uturn.forward",command:"edit.redo"),
            RadialMenuTool(title:"Закрыть",icon:"xmark",command:"navigation.close")])
    ]
}
...
@MainActor final class RadialMenuSettingsStore: ObservableObject {
    ...
    func reset(){settings=RadialMenuSettings()}
    private func save(){guard let data=try? JSONEncoder().encode(settings) else{return};UserDefaults.standard.set(data,forKey:key)}
}
...
enum RadialMenuGeometry {
    ...
    static func panelIndex(for dx:Double,dy:Double,settings s:RadialMenuSettings)->Int?{let panels=enabledPanels(s);guard !panels.isEmpty,hypot(dx,dy)>=s.deadZone else{return nil};let angle=normalizedAngle(atan2(-dy,dx)+Double.pi/2);let sector=Double.pi*2/Double(panels.count);return min(Int((angle/sector).rounded(.down)),panels.count-1)}
    static func toolIndex(for dx:Double,dy:Double,panel:RadialMenuPanel,settings s:RadialMenuSettings)->Int?{guard hypot(dx,dy)>=s.activationRadius,!panel.tools.isEmpty else{return nil};let angle=normalizedAngle(atan2(-dy,dx)+Double.pi/2);let sector=Double.pi*2/Double(panel.tools.count);return min(Int((angle/sector).rounded(.down)),panel.tools.count-1)}
}
...
struct RadialMenuView:View{
    ...
    var body:some View{ZStack{centerCore;panelRing;if let p=selectedPanel{toolRing(panel:p)}}.frame(width:520,height:520).position(center).allowsHitTesting(false).transition(.opacity.combined(with:.scale(scale:0.88)))}
    ...
    private func toolRing(panel:RadialMenuPanel)->some View{let l=max(hypot(vector.dx,vector.dy),1);let origin=CGPoint(x:260+vector.dx/l*store.settings.submenuOffset,y:260+vector.dy/l*store.settings.submenuOffset);return ZStack{Path{path in path.move(to:CGPoint(x:260,y:260));path.addLine(to:origin)}.stroke(MirTheme.Colors.selection.opacity(0.32),lineWidth:1);Circle().stroke(MirTheme.Colors.selection.opacity(0.32),lineWidth:1).frame(width:116,height:116).position(origin);ForEach(Array(panel.tools.enumerated()),id:\.element.id){i,t in let selected=selectedToolIndex==i;radialButton(title:store.settings.showLabels ? t.title:"",icon:t.icon,selected:selected,position:CGPoint(x:origin.x+CGFloat(cos(sectorAngle(index:i,count:panel.tools.count)))*52,y:origin.y+CGFloat(sin(sectorAngle(index:i,count:panel.tools.count)))*52))}}}
    private func radialButton(title:String,icon:String,selected:Bool,position:CGPoint)->some View{VStack(spacing:2){Image(systemName:icon).font(.system(size:selected ? 15:12,weight:.semibold));if !title.isEmpty{Text(title).font(.system(size:8,weight:.semibold)).lineLimit(1)}}.foregroundStyle(selected ? .white:.white.opacity(0.74)).frame(width:selected ? 78:64,height:selected ? 48:42).background(selected ? MirTheme.Colors.selection.opacity(0.78):Color.black.opacity(0.42),in:RoundedRectangle(cornerRadius:12)).overlay(RoundedRectangle(cornerRadius:12).stroke(selected ? MirTheme.Colors.selection:Color.white.opacity(0.12),lineWidth:selected ? 1.5:0.8)).scaleEffect(selected ? 1.08:1).animation(.interactiveSpring(response:0.2,dampingFraction:0.72),value:selected).position(position)}
